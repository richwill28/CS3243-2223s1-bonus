...
def astar_search(graph, h, goal_test, explored_order):
    ...
    while len(frontier) > 0:
        ...
        if goal_test(current):
            break
        for successor_node, cost in graph[current]:
            cost_fn_g[successor_node] = cost_fn_g[current] + cost
            # assuming g(n) won't get updated again, then it is safe to just append to frontier
